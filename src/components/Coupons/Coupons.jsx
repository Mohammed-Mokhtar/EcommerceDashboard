...
const couponSchema = z.object({
  code: z
    .string()
    .min(3, "code minimum length is 3 characters")
    .max(30, "code maximum length is 30 characters"),
...
export default function Coupons() {
  ...
  const [couponDeleteId, setCouponDeleteId] = useState(null);
  ...
  const handleAddCoupon = (data) => {
    ...
    axios
      .post("https://nti-ecommerce.vercel.app/api/v1/coupons", payload, {
        headers: token
  ...
  const handleDelete = () => {
    if (!couponDeleteId) return;
    const token = localStorage.getItem("token");
    axios
      .delete(
        `https://nti-ecommerce.vercel.app/api/v1/coupons/${couponDeleteId}`,
        {
          headers: token
            ? { Authorization: `Bearer ${token}`, token: token }
            : undefined,
  ...
  const openEditModal = (coupon) => {
    ...
    setEditValue("code", coupon.code || "");
    setEditValue("discount", coupon.discount || "");
    setEditValue(
      "expires",
  ...
  const handleEditCoupon = (data) => {
    ...
    axios
      .put(
        `https://nti-ecommerce.vercel.app/api/v1/coupons/${couponEditId}`,
        payload,
        {
          headers: token
            ? { Authorization: `Bearer ${token}`, token: token }
    ...
    <>
        ...
        >
          <div className="relative p-4 w-full max-w-md max-h-full">
            <div className="relative bg-neutral-primary-soft border border-default rounded-base shadow-sm p-4 md:p-6">
              ...
              <form onSubmit={handleAddSubmit(handleAddCoupon)}>
                <div className="grid gap-4 grid-cols-2 py-4 md:py-6">
                  ...
                  <div className="col-span-1">
                    ...
                    <input
                      ...
                      className="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body"
                      placeholder="10"
                      {...addRegister("discount")}
                    />
                    {addErrors.discount && (
                      <p className="mt-1 text-sm text-danger">
                        {addErrors.discount.message}
                      </p>
                    )}
                  </div>
      ...
      >
        <div className="relative p-4 w-full max-w-md max-h-full">
          <div className="relative bg-neutral-primary-soft border border-default rounded-base shadow-sm p-4 md:p-6">
            <div className="flex items-center justify-between border-b border-default pb-4 md:pb-5">
              <h3 className="text-lg font-medium text-heading">Edit coupon</h3>
              <button
                type="button"
                className="text-body bg-transparent hover:bg-neutral-tertiary hover:text-heading rounded-base text-sm w-9 h-9 ms-auto inline-flex justify-center items-center"
                onClick={() => {
            ...
            <form onSubmit={handleEditSubmit(handleEditCoupon)}>
              <div className="grid gap-4 grid-cols-2 py-4 md:py-6">
                <div className="col-span-2">
                  <label
                ...
                <div className="col-span-1">
                  ...
                  <input
                    ...
                    className="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body"
                    placeholder="10"
                    {...editRegister("discount")}
                  />
                  {editErrors.discount && (
                    <p className="mt-1 text-sm text-danger">
                      {editErrors.discount.message}
                    </p>
                  )}
                </div>
                ...
                <div className="col-span-1">
                  ...
                  <input
                    ...
                    className="bg-neutral-secondary-medium border border-default-medium text-heading text-sm rounded-base focus:ring-brand focus:border-brand block w-full px-3 py-2.5 shadow-xs placeholder:text-body"
                    {...editRegister("expires")}
                  />
                  {editErrors.expires && (
                    <p className="mt-1 text-sm text-danger">
                      {editErrors.expires.message}
                    </p>
                  )}
                </div>
              </div>
      ...
      {/* addCoupon Button */}
      <button
        data-modal-target="addCouponModal"
      ...
      <div>
        <div className="relative overflow-x-auto bg-neutral-primary-soft shadow-xs rounded-base border border-default">
          <table className="w-full text-sm text-left rtl:text-right text-body">
            ...
            <tbody>
              {(coupons || []).map((coupon) => {
                const rowId = coupon._id || coupon.id;
                const expireValue =
                  coupon.expires || coupon.expire || coupon.expiryDate;
                ...
                return (
                  <tr
                    key={rowId}
                    onClick={() => {
                      window.location.href = `/coupons/${rowId}`;
                    }}
                  ...
                  >
                    ...
                    <td className="px-6 py-4">{expireValue || "-"}</td>
